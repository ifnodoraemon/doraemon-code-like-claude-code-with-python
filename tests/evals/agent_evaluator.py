...
class AgentEvaluator:
    ...
    def evaluate_tool_usage(self, actual_tools: list[str], expected_tools: list[str]) -> dict:
        """评估工具使用"""
        return {
            "expected_tools": expected_tools,
            "actual_tools": actual_tools,
            "tool_selection_accuracy": self.calculate_tool_accuracy(actual_tools, expected_tools),
            "tool_count": len(actual_tools),
        }

    def calculate_tool_accuracy(self, actual: list[str], expected: list[str]) -> float:
        """计算工具选择准确率"""
        if not expected:
            return 1.0
    ...
    def print_report(self, report: dict):
        ...
        for difficulty, stats in sorted(report["by_difficulty"].items()):
            success_rate = stats["success"] / stats["total"] * 100
            print(f"  难度 {difficulty}: {stats['success']}/{stats['total']} ({success_rate:.1f}%)")
...
def main():
    """主函数"""
    # 加载评估任务
    AgentEvaluator("tests/evals/tasks/agent_eval_tasks.json")
