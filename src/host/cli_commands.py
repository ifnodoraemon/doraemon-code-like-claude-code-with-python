...
from src.core.commands import CommandCategory, get_registry, register_command
...
async def cmd_help(args: list, context: dict):
    ...
    console.print(
        Panel(
            Markdown(help_text), title="[bold cyan]Polymath Help[/bold cyan]", border_style="cyan"
        )
    )
    return True
...
async def cmd_tasks(args: list, context: dict):
    ...
    if len(args) > 1:
        try:
            ...
            console.print(
                "[dim]Valid statuses: pending, in_progress, completed, blocked, cancelled[/dim]"
            )
            return True
    ...
    for task in tasks:
        ...
        table.add_row(status_icon, task.id, task.title, task.priority.value)

    console.print(table)
    return True


@register_command(name="/tasks-clear", description="Clear all tasks", category=CommandCategory.TASK)
...
@register_command(
    name="/debug", description="Show debug information", category=CommandCategory.DEBUG
)
...
"""
    console.print(
        Panel(Markdown(info), title="[bold yellow]Debug Info[/bold yellow]", border_style="yellow")
    )
    return True
